...
export const TimelineUpdate = ({ updates }) => {
  moment.locale("es");
    ...
    const items = updates.map((update) => {
      ...
      item.children =
        STATES[update.state] === "Requerido" ? (
          ...
          <>
            <span style={{ margin: "0 10px 0 0" }}>{update.description}</span>
            <Image width={30} src={`data:image/png;base64,${update.photo}`} />
          </>
        ) : (
          update.description
        );
  ...
  return <Timeline mode={mode} items={items} />;
};

TimelineUpdate.propTypes = {
